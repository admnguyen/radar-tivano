...
class AircraftForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'show_in_schedule': forms.CheckboxInput(attrs={
                'class': 'form-check-input',
            }),
        }
        labels = {
            ...
            'is_active': 'Aktywny',
            'show_in_schedule': 'Widoczny w harmonogramie',
        }


class AircraftReservationForm(forms.ModelForm):
    """Formularz rezerwacji samolotu"""

    class Meta:
        model = AircraftReservation
        fields = ['aircraft', 'pilot', 'start_datetime', 'end_datetime', 'notes']
        widgets = {
            'aircraft': forms.Select(attrs={'class': 'form-select', 'required': True}),
            'pilot': forms.Select(attrs={'class': 'form-select', 'required': True}),
            ...
            'notes': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'Opcjonalne uwagi'}),
        }
        labels = {
            'aircraft': 'Samolot',
            'pilot': 'Pilot',
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['aircraft'].queryset = Aircraft.objects.filter(is_active=True)
        self.fields['aircraft'].label_from_instance = lambda obj: f"{obj.registration_marks} ({obj.manufacturer} {obj.aircraft_type})"
        self.fields['pilot'].queryset = Pilot.objects.filter(is_active=True).select_related('user')
        self.fields['pilot'].label_from_instance = lambda obj: obj.user.get_full_name()
        # Ensure datetime-local format on initial values
        self.fields['start_datetime'].input_formats = ['%Y-%m-%dT%H:%M', '%Y-%m-%d %H:%M']
        self.fields['end_datetime'].input_formats = ['%Y-%m-%dT%H:%M', '%Y-%m-%d %H:%M']
    ...
    def clean(self):
        ...
        if start and end and aircraft:
            from django.core.exceptions import ValidationError
            if end <= start:
                raise ValidationError({'end_datetime': 'Czas zakończenia musi być późniejszy niż czas rozpoczęcia.'})
            overlapping = AircraftReservation.objects.filter(
                aircraft=aircraft,
            ...
            if overlapping.exists():
                ...
                s = timezone.localtime(conflict.start_datetime)
                e = timezone.localtime(conflict.end_datetime)
                raise forms.ValidationError(
                    f'Samolot jest już zarezerwowany w tym czasie przez '
                    f'{conflict.pilot.user.get_full_name()} ({s:%d.%m.%Y %H:%M}–{e:%H:%M}).'
                )
        return cleaned_data
