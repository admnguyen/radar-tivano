...
def aircraft_photo_path(instance, filename):
    """UUID-based filename — hides original name, prevents collisions, S3-safe."""
    ext = os.path.splitext(filename)[1].lower()
    return f'aircraft_photos/{uuid.uuid4().hex}{ext}'


def pdt_photo_path(instance, filename):
    ext = os.path.splitext(filename)[1].lower()
    return f'pdt_photos/{uuid.uuid4().hex}{ext}'
...
class Aircraft(models.Model):
    ...
    show_in_schedule = models.BooleanField(default=True, verbose_name="Widoczny w harmonogramie")
...
class AircraftReservation(models.Model):
    """Rezerwacja samolotu dla pilota"""
    aircraft = models.ForeignKey(
        'Aircraft',
        on_delete=models.PROTECT,
        ...
        if self.end_datetime and self.start_datetime:
            ...
            if overlapping.exists():
                ...
                raise ValidationError(
                    ...
                    f'{conflict.pilot.user.get_full_name()} '
                    f'({s:%d.%m.%Y %H:%M}–{e:%H:%M}).'
                )

    def __str__(self):
        from django.utils import timezone
        s = timezone.localtime(self.start_datetime)
        e = timezone.localtime(self.end_datetime)
        return (
            f"{self.aircraft.registration_marks} — "
            f"{self.pilot.user.get_full_name()} "
            f"({s:%d.%m.%Y %H:%M}–{e:%H:%M})"
        )
