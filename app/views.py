        ...
        if user_form.is_valid() and pilot_form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    messages.success(
                        request,
                        f'Pilot {user.get_full_name()} został dodany. '
                        f'Hasło tymczasowe (przekaż pilotowi przez bezpieczny kanał, '
                        f'poproś o zmianę po pierwszym logowaniu): {temp_password}'
                    )
...
def download_pdt_photo(request, pk):
    """Pobieranie zdjęcia strony PDT. Pilot tylko swoje, admin wszystkie."""
    pdt_page = get_object_or_404(PDTPage, pk=pk)

    # Pilot may only download photos for PDTs they participated in. Use 404
    # (not 403) to avoid leaking existence of other pilots' PDT IDs.
    pilot = _pilot_profile(request.user)
    if pilot and not pdt_page.flight_operations.filter(pilot=pilot).exists():
        raise Http404
    ...
    if not pdt_page.photo:
        raise Http404
    ext = os.path.splitext(pdt_page.photo.name)[1].lower() or '.jpg'
    download_name = f'PDT_{pdt_page.page_number}_{pdt_page.pdt_date.isoformat()}{ext}'
    try:
        return FileResponse(pdt_page.photo.open('rb'), as_attachment=True, filename=download_name)
